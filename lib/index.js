...
function getDefaultVersionNumber(browserName, optionalRemote) {
  let versions = intersect(
    ['browserstack', 'saucelabs', 'testingbot']
      .filter(remote => browsers[remote][browserName])
      .map(remote => Object.keys(browsers[remote][browserName]))
  );
  if (versions.length === 0 && optionalRemote) {
    versions = Object.keys(browsers[optionalRemote][browserName]);
  }
  let maxVersion = -1;
  let maxVersionStr = '';
...
function getDefaultOS(browserName, versionNumber, optionalRemote) {
  let availableOperatingSystems = intersect(
    ['browserstack', 'saucelabs', 'testingbot']
      .filter(
        remote =>
          browsers[remote][browserName] &&
          browsers[remote][browserName][versionNumber]
      )
